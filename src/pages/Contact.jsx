...
const details=[{icon:Mail,label:"Email",value:"hello@gharkhoj.np"},{icon:Phone,label:"Phone",value:"+977 9825983379"},{icon:MapPin,label:"Office",value:"Jhapa, Nepal"},{icon:Clock,label:"Hours",value:"Sun – Fri, 10am – 6pm"}];
...
const Contact=()=>{
  ...
  return <div className="flex min-h-screen flex-col"><Navbar/><section className="border-b border-border/60 bg-secondary/40"><div className="container mx-auto px-4 py-16 md:py-20"><div className="max-w-2xl"><p className="eyebrow mb-3">{t("Get in touch")}</p><h1 className="display text-4xl md:text-5xl">{t("We're here to help you find home.")}</h1><p className="mt-5 text-muted-foreground">{t("Questions about a listing, verification, or listing your own room? Send us a message and a real person from the Ghar Khoj team will reply.")}</p></div></div></section>
    ...
    <section className="container mx-auto px-4 pb-20"><div className="mx-auto max-w-3xl"><p className="eyebrow mb-3 text-center">{t("Common questions")}</p><h2 className="display mb-6 text-center text-3xl">{t("Answers before you ask.")}</h2><Accordion type="single" collapsible className="soft-card px-6">{faqs.map((x,i)=><AccordionItem key={i} value={`faq-${i}`}><AccordionTrigger>{t(x.q)}</AccordionTrigger><AccordionContent className="leading-relaxed text-muted-foreground">{t(x.a)}</AccordionContent></AccordionItem>)}</Accordion></div></section>
  <Footer/></div>;
};
const Field=({label,type="text",value,onChange})=><div><Label>{label}</Label><Input className="mt-1" type={type} value={value} onChange={(e)=>onChange(e.target.value)}/></div>;
